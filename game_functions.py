...
def update_top_paddle(ai_settings, stats, screen, sb, left_paddle,
...
def update_balls(ai_settings, stats, screen, sb, left_paddle, right_paddle,
                 bottom_paddle, top_paddle, balls):
    ...
    for ball in balls.copy():
        ...
        if ball.rect.left <= 0 or ball.rect.right >= 1200 \
                or ball.rect.bottom >= 800:
    ...
    for ball in balls.sprites():
        if ball.collision_top_bottom(top_paddle, bottom_paddle):
            change_ball_direction_vertical(ai_settings, balls)

def check_keydown_events(event, ai_settings, screen, left_paddle,
         right_paddle, bottom_paddle, balls):
    """Respond to keypresses."""
    if event.key == pygame.K_UP:
        # Move the left_paddle up.
        # remember y axis decreases
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keydown_events(event, ai_settings, screen,
                 left_paddle, right_paddle, bottom_paddle, balls)

        elif event.type == pygame.KEYUP:
            check_keyup_events(event, left_paddle, right_paddle, bottom_paddle)
...
def check_keyup_events(event, left_paddle, right_paddle, bottom_paddle):
    if event.key == pygame.K_UP:
